...
export function DeliveryRouteMap({
  ...
  return (
    <section className="delivery-route-map" aria-label="Delivery route preview">
      <div className="delivery-route-map__header">
        <div>
          <span className="section-kicker">Delivery route</span>
          <h2>{stage}</h2>
        </div>
      </div>

      <svg
        className="delivery-route-map__canvas"
        viewBox="0 0 320 190"
        role="img"
        aria-labelledby="delivery-route-map-title delivery-route-map-description"
      >
        <title id="delivery-route-map-title">DopeCart delivery route preview</title>
        <desc id="delivery-route-map-description">
          A playful map showing the delivery partner moving from {darkStoreName} toward Your Sofa.
        </desc>
        <rect className="delivery-route-map__block delivery-route-map__block--one" x="12" y="16" width="82" height="52" rx="8" />
        <rect className="delivery-route-map__block delivery-route-map__block--two" x="110" y="132" width="80" height="42" rx="8" />
        <rect className="delivery-route-map__block delivery-route-map__block--three" x="204" y="24" width="82" height="54" rx="8" />
        <polyline className="delivery-route-map__route-shadow" points={pointsToString(route.points)} />
        <polyline className="delivery-route-map__route" points={pointsToString(route.points)} />

        {route.labels.map((label) => (
          <g className="delivery-route-map__label" key={label.text}>
            <rect x={label.x - 4} y={label.y - 13} width={label.text.length * 5.9 + 8} height="18" rx="5" />
            <text x={label.x} y={label.y}>{label.text}</text>
          </g>
        ))}

        <g className="delivery-route-map__pin delivery-route-map__pin--store" transform={`translate(${storePoint[0]} ${storePoint[1]})`}>
          <circle r="10" />
          <text y="4">S</text>
        </g>
        <g className="delivery-route-map__pin delivery-route-map__pin--home" transform={`translate(${homePoint[0]} ${homePoint[1]})`}>
          <circle r="10" />
          <text y="4">H</text>
        </g>
        {selfControlLabel ? (
          <circle
            className="delivery-route-map__signal"
            cx={selfControlLabel.x + 18}
            cy={selfControlLabel.y + 10}
            r="9"
          />
        ) : null}
        <g className={["delivery-route-map__marker", isLost ? "delivery-route-map__marker--lost" : ""].filter(Boolean).join(" ")} transform={`translate(${marker.x} ${marker.y})`}>
          <circle r="12" />
